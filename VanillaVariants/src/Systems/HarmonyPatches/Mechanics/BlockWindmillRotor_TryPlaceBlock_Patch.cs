...
public static class BlockWindmillRotor_TryPlaceBlock_Patch
{
    public static bool Prefix(BlockWindmillRotor __instance, ref bool __result, IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
    {
        ...
        foreach (BlockFacing face in hORIZONTALS)
        {
            ...
            if (world.BlockAccessor.GetBlock(pos) is IMechanicalPowerBlock block && block.HasMechPowerConnectorAt(world, pos, face.Opposite))
            {
                ...
                Block toPlaceBlock = world.GetBlock(__instance.CodeWithVariant("side", face.Opposite.Code));
